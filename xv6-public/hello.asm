...
int main(void){
  ...
  struct pstat* ps = (struct pstat*) malloc(sizeof(struct pstat));
  23:	e8 08 07 00 00       	call   730 <malloc>
  ...
  getpinfo(ps);
  30:	e8 06 04 00 00       	call   43b <getpinfo>
  for(int i=0; i<NPROC; i++) {
  35:	89 d8                	mov    %ebx,%eax
  37:	8d 9b 00 02 00 00    	lea    0x200(%ebx),%ebx
  3d:	83 c4 10             	add    $0x10,%esp
  40:	8d 90 00 03 00 00    	lea    0x300(%eax),%edx
  getpinfo(ps);
  46:	89 5d e4             	mov    %ebx,-0x1c(%ebp)
  49:	89 df                	mov    %ebx,%edi
  4b:	89 d3                	mov    %edx,%ebx
  4d:	eb 08                	jmp    57 <main+0x57>
  4f:	90                   	nop
  for(int i=0; i<NPROC; i++) {
  50:	83 c7 04             	add    $0x4,%edi
  53:	39 df                	cmp    %ebx,%edi
  55:	74 21                	je     78 <main+0x78>
    if(ps->pid[i] == mypid) {
  57:	39 37                	cmp    %esi,(%edi)
  59:	75 f5                	jne    50 <main+0x50>
      printf(1, "intial ticks for myPid:%d is %d\n", mypid, ps->ticks[i]);
  5b:	ff b7 00 01 00 00    	push   0x100(%edi)
  for(int i=0; i<NPROC; i++) {
  61:	83 c7 04             	add    $0x4,%edi
      printf(1, "intial ticks for myPid:%d is %d\n", mypid, ps->ticks[i]);
  64:	56                   	push   %esi
  ...
  for(int i=0; i<NPROC; i++) {
  74:	39 df                	cmp    %ebx,%edi
  76:	75 df                	jne    57 <main+0x57>
    }
  }


  fork();
  78:	89 5d d8             	mov    %ebx,-0x28(%ebp)
  7b:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
  // }
  ...
  fork();
  83:	e8 03 03 00 00       	call   38b <fork>
  int rc = fork();
  88:	e8 fe 02 00 00       	call   38b <fork>
        if(i==100 && rc==0) settickets(0);
  8d:	89 75 dc             	mov    %esi,-0x24(%ebp)
  90:	85 c0                	test   %eax,%eax
  int rc = fork();
  92:	89 c1                	mov    %eax,%ecx
        if(i==100 && rc==0) settickets(0);
  94:	0f 94 45 e4          	sete   -0x1c(%ebp)
  98:	0f b6 45 e4          	movzbl -0x1c(%ebp),%eax
  9c:	89 ce                	mov    %ecx,%esi
  9e:	89 5d e4             	mov    %ebx,-0x1c(%ebp)
  a1:	89 c3                	mov    %eax,%ebx
  a3:	eb 23                	jmp    c8 <main+0xc8>
  a5:	8d 76 00             	lea    0x0(%esi),%esi
  a8:	84 db                	test   %bl,%bl
  aa:	74 24                	je     d0 <main+0xd0>
  ac:	83 ec 0c             	sub    $0xc,%esp
  af:	6a 00                	push   $0x0
  b1:	e8 7d 03 00 00       	call   433 <settickets>
        if(i==200 && rc!=0) break;
        x += 1;
        sleep(1);
  b6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  bd:	e8 61 03 00 00       	call   423 <sleep>
  c2:	83 c4 10             	add    $0x10,%esp
  c5:	83 c7 01             	add    $0x1,%edi
        if(i==100 && rc==0) settickets(0);
  c8:	8d 47 ff             	lea    -0x1(%edi),%eax
  cb:	83 ff 65             	cmp    $0x65,%edi
  ce:	74 d8                	je     a8 <main+0xa8>
        if(i==200 && rc!=0) break;
  d0:	3d c8 00 00 00       	cmp    $0xc8,%eax
  d5:	75 04                	jne    db <main+0xdb>
  d7:	85 f6                	test   %esi,%esi
  d9:	75 15                	jne    f0 <main+0xf0>
        sleep(1);
  ...
  e8:	81 ff fa 00 00 00    	cmp    $0xfa,%edi
  ee:	75 d5                	jne    c5 <main+0xc5>
  ...
  f3:	8b 55 d8             	mov    -0x28(%ebp),%edx
  f6:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
  f9:	ff 75 e0             	push   -0x20(%ebp)
  fc:	8b 75 dc             	mov    -0x24(%ebp),%esi
  ff:	89 55 e4             	mov    %edx,-0x1c(%ebp)
 102:	e8 34 03 00 00       	call   43b <getpinfo>
 107:	8b 7d e4             	mov    -0x1c(%ebp),%edi
 10a:	83 c4 10             	add    $0x10,%esp
 10d:	eb 08                	jmp    117 <main+0x117>
 10f:	90                   	nop
  for(int i=0; i<NPROC; i++) {
 110:	83 c3 04             	add    $0x4,%ebx
 113:	39 fb                	cmp    %edi,%ebx
 115:	74 21                	je     138 <main+0x138>
    if(ps->pid[i] == mypid) {
 117:	39 33                	cmp    %esi,(%ebx)
 119:	75 f5                	jne    110 <main+0x110>
      printf(1, "final ticks for myPid:%d is %d\n", mypid, ps->ticks[i]);
 11b:	ff b3 00 01 00 00    	push   0x100(%ebx)
  for(int i=0; i<NPROC; i++) {
 121:	83 c3 04             	add    $0x4,%ebx
      printf(1, "final ticks for myPid:%d is %d\n", mypid, ps->ticks[i]);
 124:	56                   	push   %esi
 125:	68 4c 08 00 00       	push   $0x84c
 12a:	6a 01                	push   $0x1
 12c:	e8 cf 03 00 00       	call   500 <printf>
 131:	83 c4 10             	add    $0x10,%esp
  for(int i=0; i<NPROC; i++) {
 134:	39 fb                	cmp    %edi,%ebx
 136:	75 df                	jne    117 <main+0x117>
    }
  }
  exit();
 138:	e8 56 02 00 00       	call   393 <exit>
 ...
 45c:	89 d1                	mov    %edx,%ecx
{
 ...
 489:	0f b6 92 cc 08 00 00 	movzbl 0x8cc(%edx),%edx
 ...
 509:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 579:	ff 24 85 74 08 00 00 	jmp    *0x874(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
 ...
 687:	90                   	nop
          s = "(null)";
 688:	ba 6c 08 00 00       	mov    $0x86c,%edx
        while(*s != 0){
 ...
 6a1:	a1 80 0b 00 00       	mov    0xb80,%eax
{
 ...
 6e1:	5b                   	pop    %ebx
  freep = p;
 6e2:	89 15 80 0b 00 00    	mov    %edx,0xb80
}
 6e8:	5e                   	pop    %esi
 6e9:	5f                   	pop    %edi
 6ea:	5d                   	pop    %ebp
 ...
 719:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 71c:	89 15 80 0b 00 00    	mov    %edx,0xb80
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 739:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 73c:	8b 3d 80 0b 00 00    	mov    0xb80,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 77f:	73 4f                	jae    7d0 <malloc+0xa0>
    ...
    if(p == freep)
 781:	8b 3d 80 0b 00 00    	mov    0xb80,%edi
 ...
 7aa:	e8 f1 fe ff ff       	call   6a0 <free>
  return freep;
 7af:	8b 15 80 0b 00 00    	mov    0xb80,%edx
 ...
 7f7:	0b 00 00 
    base.s.size = 0;
 7fa:	bf 84 0b 00 00       	mov    $0xb84,%edi
    base.s.ptr = freep = prevp = &base;
 ...
 809:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 80b:	c7 05 88 0b 00 00 00 	movl   $0x0,0xb88
 812:	00 00 00 
    if(p->s.size >= nunits){
